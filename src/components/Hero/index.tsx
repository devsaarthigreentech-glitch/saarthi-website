...
export default function Hero({ heroSrc = '/images/hero/hero-illustration.png' }: Props) {
  return (
    // FIX 1: Keep bg-gray-900 to ensure content overflow on mobile is dark.
    <section className="relative w-full bg-gray-900 dark:bg-gray-900">
      
      {/* FIX 2: Changed z-index from -z-20 to z-0. 
        This keeps the image behind the text (which is z-10) but ensures it sits *in front* of the parent section's background color (bg-gray-900), making it visible.
      */}
      <div className="absolute left-0 right-0 top-0 z-0 w-screen min-h-[70vh] md:min-h-[85vh]">
        <img
          src={heroSrc}
          alt="Hero: vehicles, generators and buildings connected to GreenVision"
          className="w-full h-full object-cover" 
      ...
      {/* 3. Hero content: Use flex, min-h, and items-end to push the content block towards the bottom half of the screen. This element needs z-10 to be on top of the image (z-0). */}
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 
                    py-16 md:py-24 
                    flex flex-col justify-end 
                    min-h-[calc(70vh-5rem)] md:min-h-[calc(85vh-5rem)]">
        
        <div className="max-w-4xl">
          {/* Accent Text: Adjusted mb for spacing */}
          <div
            className="mb-3 text-xl md:text-2xl font-black uppercase tracking-wider"
            style={{ color: 'var(--accent, #98C948)', textShadow: "0 1px 3px rgba(0,0,0,0.5)" }}
          >
            Smart Hydrogen. Cleaner Future.
          </div>

          {/* Main Headline: Font sizing remains responsive (good), added `leading-tight` for better stacking on small screens. */}
          <h1 
