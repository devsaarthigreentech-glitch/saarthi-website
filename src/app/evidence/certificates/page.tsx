...
const patents = [
  ...
  },
  {
    id: 3,
    title: "ELECTROLYSER UNIT, SYSTEM AND METHOD OF WORKING OF ELECTROLYSER UNIT",
    link: "/downloads/patent-electrolyser.pdf",
    type: "pdf"
  },
];
...
// --- COMPONENTS ---
const CertificationCard = ({ item }) => (
  <div className="flex flex-col p-5 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 h-full">
    <div className="flex items-center mb-3">
      <div className="p-2 rounded-full bg-green-50 dark:bg-green-900/30 mr-3">
        <IconCheckCircle className="w-5 h-5" style={{ color: PRIMARY_COLOR }} />
      </div>
...
const PatentCard = ({ item }) => (
  <div className="flex flex-col sm:flex-row justify-between p-5 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700">
    <div className="pr-4">
      <div className="flex items-center mb-2">
        <IconScrollText className="w-5 h-5 mr-2" style={{ color: ACCENT_COLOR }} />
    ...
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg border border-gray-200 dark:border-slate-600 text-gray-600 dark:text-gray-300 hover:text-green-600"
    >
      Download PDF <IconDownload className="w-4 h-4 ml-2" />
    </a>
  </div>
);
...
const CertificationsPageCompact = () => {
  return (
    <>
      {/* HERO — FULL WIDTH (NO WHITE PATCH) */}
      <section className="relative bg-slate-900 text-white">
        <div className="container mx-auto px-6 py-20 md:py-24 text-center max-w-5xl">
          <p className="text-sm font-semibold uppercase tracking-widest mb-3" style={{ color: PRIMARY_COLOR }}>
            Innovation & Compliance
          </p>
      ...
      {/* PAGE CONTENT */}
      <main className="min-h-screen bg-gray-50 dark:bg-slate-950 py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto">
          ...
          <section className="mb-16">
            ...
            <div className="space-y-4">
              {patents.map(p => <PatentCard key={p.id} item={p} />)}
            </div>
          </section>
          ...
          <section className="mb-16">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {certifications.map(c => <CertificationCard key={c.id} item={c} />)}
            </div>
          </section>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-8 text-center border border-gray-100 dark:border-slate-700">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-6">
              Our compliance team is available to assist with detailed audits.
            </p>
            <a href="/contact" className="inline-block px-6 py-2.5 text-sm font-bold text-white rounded-full" style={{ backgroundColor: PRIMARY_COLOR }}>
              Contact Team
            </a>
          </div>
